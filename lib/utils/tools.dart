...
import 'package:sukar/presintation/widgets/text/headline5.dart';
import 'package:sukar/presintation/widgets/text/headline6.dart';
import 'package:sukar/utils/android_loading.dart';
import 'package:sukar/utils/exptions.dart';
import 'package:sukar/utils/ios_loading.dart';
...
class AppTools {
  ...
  String? passwordSetValidate(TextEditingController controller) {
    ...
    if (controller.text.isEmpty) {
      ...
      return 'this_password_must_be_at_least_8chrecters ';
    } else if (regExp.hasMatch(controller.text) == false) {
  ...
  String? confirmPasswordValidate(
    ...
    if (password.text.isNotEmpty) {
      if (!passwordChecker(password.text, password, confirmPass)) {
  ...
  String? phoneNumberValidate(TextEditingController controller) {
    ...
    if (!controller.text.contains(RegExp(RegExption.onlyNumber))) {
  ...
  Future<void> showCustomBottomSheet(
    ...
    await showModalBottomSheet(
      ...
      builder: (BuildContext newContext) => SingleChildScrollView(child: child),
    );
  }
  ...
  void showLoading() {
    if (Platform.isIOS) {
      showDialog(
        ...
        context: Get.context!,
        builder: (_) => const Center(child: IosLoading()),
    ...
    showCupertinoModalPopup<void>(
      ...
      builder:
          (BuildContext context) => CupertinoAlertDialog(
            title: Center(child: Headline5(title: title ?? 'alert'.tr)),
            content: Headline6(
